...
class Generator(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
        ...
        self.x = tf.placeholder(tf.float32, shape=[self.batch_size, self.sequence_length, 3]) # sequence of tokens generated by generator
        ...
        with tf.device("/cpu:0"):
            self.processed_x = tf.transpose(tf.concat([self.x[:, :, :2], tf.nn.embedding_lookup(self.g_embeddings, tf.cast(self.x[:, :, 2], tf.int32))], axis=2), perm=[1, 0, 2])  # seq_length x batch_size x emb_dim
        ...
        _, _, _, self.gen_o, self.gen_x = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.concat([self.start_continuous_token, tf.nn.embedding_lookup(self.g_embeddings, self.start_category_token)], axis=1),
                       self.h0, gen_o, gen_x))
