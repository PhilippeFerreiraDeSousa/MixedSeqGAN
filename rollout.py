...
class ROLLOUT(object):
    def __init__(self, lstm, update_rate):
        ...
        self.start_continuous_token = tf.identity(self.lstm.start_continuous_token)
        self.start_category_token = tf.identity(self.lstm.start_category_token)
        ...
        with tf.device("/cpu:0"):
            self.processed_x = tf.transpose(a=tf.concat([self.x[:, :, :2], tf.nn.embedding_lookup(params=self.g_embeddings, ids=tf.cast(self.x[:, :, 2], tf.int32))], axis=2), perm=[1, 0, 2])  # seq_length x batch_size x (emb_dim + 2)

        ta_emb_x = tensor_array_ops.TensorArray(
            dtype=tf.float32, size=self.sequence_length)
        ta_emb_x = ta_emb_x.unstack(self.processed_x)

        ta_x = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length)
        ta_x = ta_x.unstack(tf.transpose(a=self.x, perm=[1, 0, 2]))
        #####################################################################################################
        ...
        def _g_recurrence_2(i, x_t, h_tm1, given_num, gen_x):
            ...
            log_prob = tf.math.log(tf.nn.softmax(o_t[:, 2:]))
            next_token = tf.reshape(tf.random.categorical(logits=log_prob, num_samples=1, dtype=tf.int32), [self.batch_size])
            x_tp1 = tf.concat([o_t[:, :2], tf.nn.embedding_lookup(params=self.g_embeddings, ids=next_token)], axis=1)  # batch x (emb_dim + 2)
        ...
        self.gen_x = tf.transpose(a=self.gen_x, perm=[1, 0, 2])  # batch_size x seq_length x 3
